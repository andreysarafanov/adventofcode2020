import { evaluate, evaluateWithPrecedence } from "./math-operators/math-operators";

const input = `5 + 9 + 3 + ((2 + 8 + 2) + 8 + 9 * (4 * 2 * 5) + 6) * 4
...
console.log(input.split('\n').map(evaluate).reduce((a, b) => a + b, 0));

console.log(input.split('\n').map(evaluateWithPrecedence).reduce((a, b) => a + b, 0));
